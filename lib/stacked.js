...
async function loadNote(slug) {
  ...
  // In development, always refetch so markdown edits are visible after refresh
  // or navigation without restarting the dev server.
  if (!isDev && noteCache.has(bare)) return noteCache.get(bare);
  ...
  try {
    ...
    if (!isDev) noteCache.set(bare, data);
    return data;
  } catch (e) {
    return null;
  }
