...
import { hydrateNotePayload } from './note-payload.mjs';

// ---- Contexts ----
const StackedPagesContext = createContext({
  stackedPages: [],
  stackedPageStates: {},
  navigateToStackedPage: () => {},
  highlightStackedPage: () => {},
});
const StackedPagesIndexContext = createContext(0);
...
let noteMetaPromise = null;
let articleNotesPromise = null;
const DEV_NOTE_CACHE_TTL_MS = 120000;

function cacheNote(slug, data) {
  const isDev = process.env.NODE_ENV === 'development';
  noteCache.set(slug, {
    data,
    expiresAt: isDev ? Date.now() + DEV_NOTE_CACHE_TTL_MS : Number.POSITIVE_INFINITY,
  });
}

async function loadNoteMeta() {
  const isDev = process.env.NODE_ENV === 'development';
  if (isDev) {
    const url = `/note-meta.json?t=${Date.now()}`;
    return fetch(url, { cache: 'no-store' })
      .then((res) => (res.ok ? res.json() : {}))
      .catch(() => ({}));
  }

  if (noteMetaPromise) return noteMetaPromise;

  noteMetaPromise = fetch('/note-meta.json')
    .then((res) => (res.ok ? res.json() : {}))
    .catch(() => ({}));
  return noteMetaPromise;
}
...
async function loadNote(slug) {
  ...
  const isDev = process.env.NODE_ENV === 'development';
  const cached = noteCache.get(bare);

  // Return cached data to avoid redundant fetches within the same session.
  // In development, keep the cache short-lived enough for content edits but
  // long enough for idle/hover prefetches to make repeated clicks instant.
  if (cached && (!isDev || cached.expiresAt > Date.now())) return cached.data;
  ...
  try {
    ...
    const noteMeta =
      payload?.h || (!(payload?.o?.length) && !(payload?.i?.length)) ? null : await loadNoteMeta();
    const data = hydrateNotePayload(payload, bare, noteMeta);
    cacheNote(bare, data);
    return data;
  } catch (e) {
    return null;
...
async function loadArticleNotesBundle() {
  const isDev = process.env.NODE_ENV === 'development';
  if (articleNotesPromise && !isDev) return articleNotesPromise;

  articleNotesPromise = (async () => {
    try {
      const url = `/article-notes.json${isDev ? `?t=${Date.now()}` : ''}`;
      const res = await fetch(url, { cache: isDev ? 'no-store' : 'default' });
      if (!res.ok) return false;
      const payloads = await res.json();
      const noteMeta = await loadNoteMeta();
      Object.entries(payloads).forEach(([slug, payload]) => {
        const data = hydrateNotePayload(payload, slug, noteMeta);
        if (data) cacheNote(slug, data);
      });
      return true;
    } catch (e) {
      return false;
    } finally {
      if (isDev) articleNotesPromise = null;
    }
  })();

  return articleNotesPromise;
}

function readCachedNote(slug) {
  const bare = String(slug).replace(/^\/+/, '').replace(/\/+$/, '');
  const isDev = process.env.NODE_ENV === 'development';
  const cached = noteCache.get(bare);
  if (cached && (!isDev || cached.expiresAt > Date.now())) return cached.data;
...
function buildPendingPage(slug) {
  return {
    slug,
    data: readCachedNote(slug) || {
      slug,
      title: 'กำลังโหลด...',
      isLoading: true,
...
export function prefetchNote(slug) {
  return loadNote(slug);
}

export async function prefetchNotes(slugs, batchSize = 8) {
  const uniqueSlugs = [...new Set(slugs.filter(Boolean))];
  for (let i = 0; i < uniqueSlugs.length; i += batchSize) {
    await Promise.all(uniqueSlugs.slice(i, i + batchSize).map((slug) => loadNote(slug)));
  }
}

export function prefetchArticleNotes() {
  return loadArticleNotesBundle();
}
...
export function useStackedPagesProvider({ location, firstPage, navigate, pageWidth = 625, obstructedPageWidth = 40 }) {
  ...
  useEffect(() => {
    if (firstPage === previousFirstPage.current) return;
  ...
  useEffect(() => {
    let cancelled = false;
    const pendingPages = stackedPagesSlugs.map(buildPendingPage);
    setStackedPages(getRoot(firstPage).concat(pendingPages));
    if (pendingPages.every((page) => !page.data?.isLoading)) {
      return () => {
        cancelled = true;
      };
    }
