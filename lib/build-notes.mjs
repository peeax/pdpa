...
import { buildFullActContent, validateActStructure } from './full-act.mjs';
import {
  extractConsultationMetadata,
  removeConsultationTitle,
} from './consultation-metadata.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
// lib/ -> content/
const CONTENT_DIR = path.resolve(__dirname, '..', 'content');
const ACT_STRUCTURE_PATH = path.join(CONTENT_DIR, 'act-structure.json');
const DISCUSSION_REDIRECTS_PATH = path.join(CONTENT_DIR, 'discussion-redirects.json');
...
export function buildAll() {
  ...
  const redirects = fs.existsSync(DISCUSSION_REDIRECTS_PATH)
    ? JSON.parse(fs.readFileSync(DISCUSSION_REDIRECTS_PATH, 'utf8'))
    : {};
  ...
  const articleNumbers = validateActStructure(actStructure);
  ...
  const fullActMtimes = [
    ...
    ...articleNumbers.map((number) => slugToNote[`article-${number}`]?.mtime),
  ].filter(Boolean);
  ...
  if (fs.existsSync(referencesDir)) {
    ...
    for (const filename of referenceFiles) {
      const referenceSlug = generateSlug(path.parse(filename).name);
      const raw = fs.readFileSync(path.join(referencesDir, filename), 'utf-8');
      const { content, data: frontmatter } = matter(raw);
      if (!frontmatter.title) continue;
      const trimmedContent = content.trim();
      // Reference files can link to titles and aliases already registered from
      // the primary content directory. Reference-to-reference links are not
      // registered here and therefore remain plain text for now.
      const linkedContent = insertLinks(trimmedContent, nameToSlugMap, ROOT_PATH, true);
      notes.set(referenceSlug, {
        slug: referenceSlug,
        title: frontmatter.title,
        isReference: true,
        ...
        excerpt: makeExcerpt(linkedContent),
      });
    }
  }

  // --- Discussion / consultation notes (content/discussion/*.md) ---
  // The filename remains the public slug. Frontmatter owns display metadata
  // and article relationships so those links have one source of truth.
  ...
  if (fs.existsSync(discussionDir)) {
    ...
    for (const filename of discussionFiles) {
      ...
      const relatedArticles = frontmatter.articles || [];
      if (
        !Array.isArray(relatedArticles) ||
        relatedArticles.some((article) => !Number.isInteger(article))
      ) {
        throw new Error(`${path.relative(CONTENT_DIR, filePath)} must use an integer articles array`);
      }

      // BrainNote renders the title, so keep it out of the Markdown body.
      const bodyContent = removeConsultationTitle(content, metadata.titleSource);
      const body = insertLinks(bodyContent, nameToSlugMap, ROOT_PATH, true);
      ...
      notes.set(slug, {
        ...
        isHighlight: false,
        body,
        excerpt: metadata.summary || makeExcerpt(body),
        mtime,
        definedTerms: [],
  ...
  const relatedConsultationsByArticle = {};
  for (const note of notes.values()) {
    for (const articleNumber of note._relatedArticles || []) {
      const articleSlug = `article-${articleNumber}`;
      const articleNote = notes.get(articleSlug);
      if (!articleNote) {
        throw new Error(`${note.slug} references missing ${articleSlug}`);
      }
      const reference = refShape(note.slug);
      articleNote.relatedConsultations = articleNote.relatedConsultations || [];
      articleNote.relatedConsultations.push(reference);
      relatedConsultationsByArticle[articleNumber] =
        relatedConsultationsByArticle[articleNumber] || [];
      relatedConsultationsByArticle[articleNumber].push(reference);
      (backlinkMap[note.slug] = backlinkMap[note.slug] || []).push(articleSlug);
    }
    delete note._relatedArticles;
  }

  const sortConsultations = (a, b) =>
    a.title.localeCompare(b.title, 'th', { numeric: true });
  for (const consultations of Object.values(relatedConsultationsByArticle)) {
    consultations.sort(sortConsultations);
  }
  for (const note of notes.values()) {
    if (note.relatedConsultations) note.relatedConsultations.sort(sortConsultations);
  }
  notes.get(ROOT_NOTE).relatedConsultationsByArticle = relatedConsultationsByArticle;
...
export function getRedirectTarget(slug) {
  return buildAll().redirects[slug] || null;
}

export function getAllRouteSlugs() {
  const { notes, redirects } = buildAll();
  return [...new Set([...notes.keys(), ...Object.keys(redirects)])];
}
