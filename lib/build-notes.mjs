...
export function buildAll() {
  ...
  for (const slug in slugToNote) {
    ...
    // targets already written in the body (e.g. links to highlight/announcement
    // popups). Including both means highlight links get hover popovers too, not
  ...
  if (fs.existsSync(highlightsDir)) {
    ...
    for (const filename of highlightFiles) {
      ...
      notes.set(frontmatter.slug, {
        ...
        // Excerpt so hover popovers work for highlight/announcement links too.
