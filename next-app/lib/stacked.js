...
// ---- Fetch a note's data, mirroring Gatsby's lazy loadPage ----
// Link targets carry a leading slash (e.g. "/article-1"); the JSON files are
// named by bare slug ("article-1.json"), so normalize before fetching.
async function loadNote(slug) {
  const bare = String(slug).replace(/^\/+/, '').replace(/\/+$/, '');
  try {
    const res = await fetch(`/notes/${encodeURIComponent(bare)}.json`);
